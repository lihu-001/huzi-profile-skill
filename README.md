...
1. **`.huzi-profile/profile-preview.png` (749×465)**: Assembled X desktop dark-mode preview card combining the banner, overlapping circular avatar, verified badge, buttons ("编辑个人资料", "$"), display name, handle, and bio.
...
The card can be composed automatically using the bundled Python script (defaults read from and write to `.huzi-profile/`):
```bash
python compose_preview.py --name "HUZI" --handle "@lihu9048" --bio "学习AI，分享AI"
```
